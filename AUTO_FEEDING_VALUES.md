# Autonomous Feeding Values

This table records the measured and selected values used by the automatic algae
feeding mode.

## Highest Priority

These values are now filled into `controller_config.py`.
...
| `OD_LINEAR_SLOPE` | Linear conversion from raw OD count to cells/mL | `-72.223` | Final inverted OD calibration |
| `OD_LINEAR_INTERCEPT` | Linear calibration offset | `3000000.0` | Final inverted OD calibration |
| `OD_BLANK_RAW` | Clean-water sensor reading for absorbance mode | `45000.0` | Measured clean-water value |
...
| `ALGAE_STOCK_CONCENTRATION` | Algae concentration in feed tank | `200000.0` | Measured in cells/mL |
| `FEED_TARGET_CONCENTRATION` | Desired algae concentration in mussel tank | `1338.0` | Selected biological target |
| `TANK_VOLUME_L` | Actual water volume in mussel tank | `4.0` | Measure real filled volume |
| `MUSSEL_CLEARANCE_RATE_L_H` | Clearance rate per mussel | `3.5` | Measured/selected model value |
...
| --- | --- | --- | --- |
| `FEED_LOW_FRACTION` | Lower feeding threshold as fraction of target | `0.90` | With target `1338`, feeds below about `1204` |
| `FEED_HIGH_FRACTION` | Upper stop/hold threshold as fraction of target | `1.10` | With target `1338`, holds above about `1472` |
...
| `FEED_DOSE_COOLDOWN_SECONDS` | Minimum wait between new doses | `600` | 10 minutes, matching the feeding model interval |
| `FEED_MAX_COOLDOWN_SECONDS` | Maximum wait between new doses | `900` | 15 minutes; gives a minimum pulse unless OD is above the high threshold |
